        ...
        for col in cols:
            ...
            if col['key'] == 'clients':
                for client in col['value']:
                    ...
                    self.ws.cell(row=self.last_row, column=col_index, value=name)
                    col_index += 1
                    self.ws.cell(row=self.last_row, column=col_index,
                    ...
                    col_index += 1
                continue
            self.ws.cell(row=self.last_row, column=col_index, value=col['value'])
        self.last_row += 1
        self.save()
    ...
    def create_header(self, starting_row):
        ...
        for col in cols:
            ...
            if row_index > last_row_i:
                col_index -= 1
            self.ws.cell(row=row_index, column=col_index, value=col['key'])
